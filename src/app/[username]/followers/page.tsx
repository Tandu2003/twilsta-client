...
export default function FollowersPage() {
  ...
  const { getFollowers, getFollowersForUser, isLoading: followLoading } = useFollow();
  ...
  const followersData = user ? getFollowersForUser(user.id) : null;
  ...
  useEffect(() => {
    const loadFollowers = async () => {
      if (!user || hasLoadedFollowers) return;

      try {
        await getFollowers(user.id, { limit: 20 });
    ...
    loadFollowers();
  }, [user, getFollowers, hasLoadedFollowers]);

  const handleLoadMore = async () => {
    if (!user || !followersData?.pagination.hasNext) return;
        ...
        <CardHeader>
          <div className='flex items-center space-x-4'>
            ...
            <div className='flex items-center space-x-2'>
              ...
              <CardTitle>{isOwnProfile ? 'Your Followers' : `@${username}'s Followers`}</CardTitle>
            </div>
          </div>
        </CardHeader>

        <CardContent>
          {followLoading && !hasLoadedFollowers ? (
            ...
            <div className='py-8 text-center'>
              ...
              <h3 className='mb-2 text-lg font-semibold text-gray-700'>No followers yet</h3>
                ...
                >
                  ...
                  >
                    <Avatar className='h-12 w-12'>
                      ...
                      <AvatarFallback>
                        {follower.fullName?.charAt(0) || follower.username.charAt(0).toUpperCase()}
                      </AvatarFallback>
                    </Avatar>
                  ...
                  {/* Follow button - only show if not own profile and not viewing own follower */}
                  {isAuthenticated && !isOwnProfile && currentUser?.id !== follower.id && (
                    <ProfileFollowButton
